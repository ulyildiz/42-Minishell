	...
	*/
	...
	if (cmds->value[1] != NULL)
	{
		if(cmds->value[1][0] != '-')
		{
			...
			cd_path = ft_strappend(cd_path,cmds->value[1],ft_strlen(cmds->value[1]));
		}
...
void pwd(t_command *cmds, t_main *shell)
{
	...
	if(pwd != NULL)
	{
		...
		ft_putstr_fd("\n",cmds->fd[1]);//normal bash ile karşılaştırdım newline atıyor rdr de bile
...
void env(t_command *cmds, t_main *shell)
{
	...
	{	
		...
		ft_putstr_fd("\n",cmds->fd[1]); // eğer value ya sahip olmayan nameler varsa = den sonra direkt newline atmıyordu ondan if dışında
		env = env->next;
	}
}
